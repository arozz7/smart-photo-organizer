...
import electron, { app as app$1, BrowserWindow, protocol, net, ipcMain as ipcMain$1, dialog } from "electron";
          ...
          if (![".tif", ".tiff"].includes(ext)) {
            try {
              ...
              if (tool) {
                ...
                await sharp(tempPreviewPath).rotate().resize(1200, 1200, { fit: "inside", withoutEnlargement: true }).jpeg({ quality: 80 }).toFile(previewPath);
                try {
...
function startPythonBackend() {
  ...
  pythonProcess.on("close", (code2) => {
    console.log(`[Main] Python process exited with code ${code2}`);
    pythonProcess = null;
  });
}
...
app$1.whenReady().then(async () => {
  ...
  ipcMain$1.handle("read-file-buffer", async (_, filePath) => {
    ...
    try {
      ...
      console.error("Failed to read file:", filePath, error2);
      throw error2;
    }
  });
  ...
  ipcMain$1.handle("ai:saveSettings", (event, settings) => {
  ...
  ipcMain$1.handle("face:getBlurry", async (event, { personId, threshold, scope: scope2 }) => {
    const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
    const db2 = getDB2();
    let query = "";
    const params = [];
  ...
  ipcMain$1.handle("settings:cleanupPreviews", async (event, { days }) => {
    try {
      const previewDir = path.join(getLibraryPath(), "previews");
      try {
        await fs.access(previewDir);
  ...
  ipcMain$1.handle("face:deleteFaces", async (event, faceIds) => {
    const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
    const db2 = getDB2();
  ...
  ipcMain$1.handle("ai:scanImage", async (_, { photoId }) => {
    const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
    const db2 = getDB2();
  ...
  ipcMain$1.handle("db:addTags", async (_, { photoId, tags }) => {
    ...
    try {
      ...
      console.error("Failed to add tags:", error2);
  ...
  ipcMain$1.handle("db:getTags", async (_, photoId) => {
    ...
    try {
      ...
      console.error("Failed to get tags:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:clearAITags", async () => {
    const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
    const db2 = getDB2();
    try {
      db2.exec(`
      ...
      console.error("Failed to clear AI tags:", error2);
      return { success: false, error: error2 };
    }
  });
  ipcMain$1.handle("db:getPhoto", async (_, photoId) => {
    ...
    try {
      ...
      console.error("Failed to get photo:", error2);
      return null;
    }
  });
  ...
  ipcMain$1.handle("db:deleteFaces", async (_, faceIds) => {
    ...
    try {
      ...
      console.error("Failed to delete faces:", error2);
      return { success: false, error: error2 };
    }
  });
