...
const logDir = path.join(app$1.getPath("userData"), "logs");
if (!fs$1.existsSync(logDir)) {
  fs$1.mkdirSync(logDir, { recursive: true });
}
const logFile = path.join(logDir, "main.log");
const MAX_SIZE = 5 * 1024 * 1024;
let logStream = fs$1.createWriteStream(logFile, { flags: "a" });
function rotateLogIfNeeded() {
  try {
    if (fs$1.existsSync(logFile)) {
      const stats = fs$1.statSync(logFile);
      if (stats.size > MAX_SIZE) {
        logStream.end();
        const oldLog = logFile + ".old";
        if (fs$1.existsSync(oldLog)) fs$1.unlinkSync(oldLog);
        fs$1.renameSync(logFile, oldLog);
        logStream = fs$1.createWriteStream(logFile, { flags: "a" });
      }
    }
  } catch (err) {
...
const store$1 = new ElectronStore({
  defaults: {
    libraryPath: path.join(app$1.getPath("userData")),
    aiSettings: {
...
function setLibraryPath(newPath) {
  store$1.set("libraryPath", newPath);
}
function getAISettings() {
  return store$1.get("aiSettings");
}
function setAISettings(settings) {
  store$1.set("aiSettings", settings);
}
let db;
...
function getFacesForClustering() {
  const db2 = getDB();
  try {
...
function getMetricsHistory(limit2 = 1e3) {
  const db2 = getDB();
  try {
    const history = db2.prepare("SELECT * FROM scan_history ORDER BY timestamp DESC LIMIT ?").all(limit2);
...
const recalculatePersonMean = (db2, personId) => {
  try {
    ...
    const allDescriptors = db2.prepare(`
...
function findBestMatch(descriptor, candidates, threshold) {
  let mag = 0;
  ...
  for (const person of candidates) {
    ...
    for (let i = 0; i < normalized.length; i++) {
      const diff2 = normalized[i] - person.mean[i];
      sumSq += diff2 * diff2;
    }
  ...
  try {
    ...
    while (pass <= MAX_PASSES) {
      ...
      if (faceIds && faceIds.length > 0) {
        ...
        faces = db2.prepare(`SELECT id, descriptor FROM faces WHERE id IN (${placeholders}) AND person_id IS NULL AND descriptor IS NOT NULL`).all(...faceIds);
      } else {
        ...
        for (const face of faces) {
          ...
          if (match.success && match.personId) {
            ...
            if (passAssignedCount <= 5) {
              logger.info(`[AutoAssign] Matched Face ${face.id} to ${match.personName} (Sim: ${(_a = match.similarity) == null ? void 0 : _a.toFixed(4)})`);
            }
          }
        }
      });
...
async function cleanupTags() {
  ...
  try {
    ...
    const getOrCreateTagId = (tagName) => {
      ...
      const existing = db2.prepare("SELECT id FROM tags WHERE name = ?").get(lowerName);
      if (existing) return existing.id;
      const info = db2.prepare("INSERT INTO tags (name) VALUES (?)").run(lowerName);
      return Number(info.lastInsertRowid);
    };
    const transaction = db2.transaction(() => {
      const getPhotoTags = db2.prepare("SELECT photo_id FROM photo_tags WHERE tag_id = ?");
      const insertPhotoTag = db2.prepare("INSERT OR IGNORE INTO photo_tags (photo_id, tag_id, source) VALUES (?, ?, ?)");
      const deleteOldTag = db2.prepare("DELETE FROM tags WHERE id = ?");
      const deleteOldLinks = db2.prepare("DELETE FROM photo_tags WHERE tag_id = ?");
      ...
      for (const tag of allTags) {
        const clean2 = tag.name.replace(/['"]/g, "");
        ...
        const affectedPhotoIds = getPhotoTags.all(tag.id).map((r) => r.photo_id);
...
async function getScanErrors(limit2 = 100) {
  const db2 = getDB();
  try {
    const errors2 = db2.prepare("SELECT * FROM scan_errors ORDER BY timestamp DESC LIMIT ?").all(limit2);
    return { success: true, errors: errors2 };
  } catch (e) {
    return { success: false, error: String(e) };
  }
}
async function deleteScanErrorAndFile(errorId, deleteFile) {
  const db2 = getDB();
  const fs2 = await import("node:fs/promises");
  try {
    const row = db2.prepare("SELECT file_path FROM scan_errors WHERE id = ?").get(errorId);
    if (deleteFile && row && row.file_path) {
      try {
        await fs2.unlink(row.file_path);
        logger.info(`[ScanErrors] Deleted corrupt file: ${row.file_path}`);
      } catch (e) {
        logger.warn(`[ScanErrors] Failed to delete file: ${row.file_path}`, e);
      }
    }
    db2.prepare("DELETE FROM scan_errors WHERE id = ?").run(errorId);
    if (row && row.file_path && deleteFile) {
      db2.prepare("DELETE FROM photos WHERE file_path = ?").run(row.file_path);
    }
    return { success: true };
  } catch (e) {
    return { success: false, error: String(e) };
...
function findPotentialMatches(faceIds, threshold = 0.4) {
  const db2 = getDB();
  try {
    const people = db2.prepare("SELECT id, name, descriptor_mean_json FROM people WHERE descriptor_mean_json IS NOT NULL").all();
    ...
    const placeholders = faceIds.map(() => "?").join(",");
    const faces = db2.prepare(`SELECT id, descriptor FROM faces WHERE id IN (${placeholders}) AND descriptor IS NOT NULL`).all(...faceIds);
    const matches = [];
    for (const face of faces) {
      const rawDescriptor = Array.from(new Float32Array(face.descriptor.buffer, face.descriptor.byteOffset, face.descriptor.byteLength / 4));
      const match = findBestMatch(rawDescriptor, candidates, threshold);
...
const db$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  cleanupTags,
  closeDB,
  deleteScanErrorAndFile,
  findBestMatch,
  findPotentialMatches,
  getDB,
  getFacesByIds,
  getFacesForClustering,
  getMetricsHistory,
  getScanErrors,
  initDB,
...
async function getExifTool() {
  ...
  _exiftoolInitPromise = (async () => {
    try {
      ...
      const initCheck = new Promise((resolve2, reject) => {
    ...
    try {
      ...
      if (tool) {
        ...
        if (tags == null ? void 0 : tags.Orientation) {
          ...
          if (val === 1 || val === "Horizontal (normal)") {
            rotationDegrees = 0;
            shouldRotate = false;
          } else if (val === 3 || val === "Rotate 180") {
            rotationDegrees = 180;
            shouldRotate = true;
          } else if (val === 6 || val === "Rotate 90 CW") {
            rotationDegrees = 90;
            shouldRotate = true;
          } else if (val === 8 || val === "Rotate 270 CW") {
            rotationDegrees = 270;
            shouldRotate = true;
          }
        }
      ...
      if (![".tif", ".tiff"].includes(ext)) {
        try {
          ...
          if (tool) {
            ...
            await pipeline.resize(1200, 1200, { fit: "inside", withoutEnlargement: true }).jpeg({ quality: 80 }).toFile(previewPath);
            try {
          ...
          try {
            await promises.unlink(`${previewPath}.tmp`);
          } catch {
          }
        }
      }
  ...
  if (photo) {
    ...
    if (!photo.metadata_json || photo.metadata_json === "{}" || forceRescan) {
      try {
        const tool = await getExifTool();
        if (tool) {
          const metadata2 = await tool.read(fullPath);
          db2.prepare("UPDATE photos SET metadata_json = ? WHERE id = ?").run(JSON.stringify(metadata2), photo.id);
          photo.metadata_json = JSON.stringify(metadata2);
          needsUpdate = true;
        }
      } catch (e) {
  ...
  if (!photo) {
    ...
    try {
      ...
      try {
        const tool = await getExifTool();
        if (tool) {
          metadata2 = await tool.read(fullPath);
        }
      } catch (e) {
      ...
      insertStmt.run({
        ...
        metadata_json: JSON.stringify(metadata2)
      });
...
function startPythonBackend() {
  ...
  pythonProcess = spawn(pythonPath, args, {
    ...
    env: {
      ...process.env,
      IS_DEV: app$1.isPackaged ? "false" : "true",
      HF_HUB_DISABLE_SYMLINKS_WARNING: "1",
      LIBRARY_PATH,
      LOG_PATH: path.join(app$1.getPath("userData"), "logs"),
      PYTORCH_CUDA_ALLOC_CONF: "expandable_segments:True"
    }
  });
  if (pythonProcess.stdout) {
    setTimeout(() => syncInitialSettings(), 2e3);
    ...
    reader.on("line", async (line) => {
      ...
      try {
        ...
        if (message.type === "cluster_result") {
          logger.info(`[Python] Cluster Result: ${((_a = message.clusters) == null ? void 0 : _a.length) || 0} clusters found.`);
        } else if (message.type === "search_result") {
          logger.info(`[Python] Search Result: ${((_b = message.matches) == null ? void 0 : _b.length) || 0} matches found.`);
        ...
        if (win && (message.type === "scan_result" || message.type === "tags_result" || message.type === "analysis_result")) {
          ...
          const isSuccess = message.type === "scan_result" && message.success || message.type === "analysis_result" && !message.error;
          if (isSuccess) {
            try {
              const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
              const db2 = getDB2();
              if (message.type === "analysis_result" && message.faces && message.faces.length > 0) {
                const existingFaces = db2.prepare("SELECT id, box_json, person_id FROM faces WHERE photo_id = ?").all(message.photoId);
                const updateFaceStmt = db2.prepare("UPDATE faces SET descriptor = ?, box_json = ?, blur_score = ? WHERE id = ?");
                ...
                if (insertedIds.length > 0) {
                  try {
                    ...
                    const matchRes = await autoAssignFaces2(insertedIds, threshold);
        ...
        if (message.type === "cluster_result") {
          console.log(`[Main] Received Cluster Result for ${message.photoId || "Batch"}. Clusters: ${(_c = message.clusters) == null ? void 0 : _c.length}`);
        }
        ...
        if (resId && scanPromises.has(resId)) {
          ...
          if (message.error) {
            promise == null ? void 0 : promise.reject(message.error);
          } else {
            promise == null ? void 0 : promise.resolve(message);
          }
        ...
        if ((message.type === "scan_result" || message.type === "tags_result") && message.error && message.photoId) {
          try {
            const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
            const db2 = getDB2();
            const logError = db2.prepare("INSERT INTO scan_errors (photo_id, file_path, error_message, stage) VALUES (?, (SELECT file_path FROM photos WHERE id = ?), ?, ?)");
  ...
  pythonProcess.on("close", (code2) => {
    logger.info(`[Main] Python process exited with code ${code2}`);
    pythonProcess = null;
  });
}
...
app$1.whenReady().then(async () => {
      ...
      if (width && width > 0 || boxParam) {
        try {
          ...
          if (boxParam) {
            ...
            if (!isNaN(x) && !isNaN(y) && !isNaN(w) && !isNaN(h)) {
              ...
              if (metadata2.width && metadata2.height) {
                ...
                if (originalWidth && originalWidth > 0 && metadata2.width !== originalWidth) {
                  const scale = metadata2.width / originalWidth;
                  finalX = x * scale;
                  finalY = y * scale;
                  finalW = w * scale;
                  finalH = h * scale;
                }
                const safeX = Math.max(0, Math.min(Math.round(finalX), metadata2.width - 1));
                const safeY = Math.max(0, Math.min(Math.round(finalY), metadata2.height - 1));
                const safeW = Math.max(1, Math.min(Math.round(finalW), metadata2.width - safeX));
                const safeH = Math.max(1, Math.min(Math.round(finalH), metadata2.height - safeY));
          ...
          try {
            ...
            if (!photoId) {
              const row = db2.prepare("SELECT id FROM photos WHERE file_path = ?").get(decodedPath);
              if (row) photoId = row.id;
            }
            const errorMsg = resizeErr.message || "Unknown Image Error";
            db2.prepare("INSERT INTO scan_errors (photo_id, file_path, error_message, stage) VALUES (?, ?, ?, ?)").run(photoId, decodedPath, errorMsg, "Preview Generation");
          } catch (dbErr) {
  ...
  createWindow();
  ipcMain$1.handle("app:focusWindow", () => {
    if (win) {
  ...
  ipcMain$1.handle("scan-directory", async (event, dirPath, options = {}) => {
    return await scanDirectory(dirPath, LIBRARY_PATH, (count) => {
      event.sender.send("scan-progress", count);
    }, options);
  });
  ipcMain$1.handle("scan-files", async (event, filePaths, options = {}) => {
    return await scanFiles(filePaths, LIBRARY_PATH, (count) => {
      event.sender.send("scan-progress", count);
    }, options);
  });
  ipcMain$1.handle("dialog:openDirectory", async () => {
  ...
  ipcMain$1.handle("read-file-buffer", async (_, filePath) => {
    const fs2 = await import("node:fs/promises");
    try {
      const buffer = await fs2.readFile(filePath);
      return buffer;
    } catch (error2) {
      logger.error("Failed to read file:", filePath, error2);
      throw error2;
    }
  });
  ipcMain$1.handle("ai:getSettings", () => {
    return getAISettings();
  });
  ipcMain$1.handle("db:getMetricsHistory", async () => {
    const { getMetricsHistory: getMetricsHistory2 } = await Promise.resolve().then(() => db$1);
    return getMetricsHistory2();
  });
  ipcMain$1.handle("db:getScanErrors", async () => {
    const { getScanErrors: getScanErrors2 } = await Promise.resolve().then(() => db$1);
    return await getScanErrors2();
  });
  ipcMain$1.handle("db:deleteScanError", async (_, { id: id2, deleteFile }) => {
    const { deleteScanErrorAndFile: deleteScanErrorAndFile2 } = await Promise.resolve().then(() => db$1);
    return await deleteScanErrorAndFile2(id2, deleteFile);
  });
  ipcMain$1.handle("ai:saveSettings", (_event, settings) => {
  ...
  ipcMain$1.handle("ai:downloadModel", async (_event, { modelName }) => {
  ...
  ipcMain$1.handle("db:cleanupTags", async () => {
    const { cleanupTags: cleanupTags2 } = await Promise.resolve().then(() => db$1);
    return await cleanupTags2();
  });
  ipcMain$1.handle("ai:getSystemStatus", async () => {
    return new Promise((resolve2, reject) => {
      const requestId = Math.floor(Math.random() * 1e6);
      scanPromises.set(requestId, {
        resolve: (res) => resolve2(res.status || {}),
        reject
      });
      sendToPython({
  ...
  ipcMain$1.handle("face:getBlurry", async (_event, { personId, threshold, scope: scope2, limit: limit2 = 1e3, offset = 0 }) => {
    const db2 = getDB();
    let query = "";
    let countQuery = "";
    const params = [];
    ...
    try {
      ...
      const totalRes = countStmt.get(...countParams);
  ...
  ipcMain$1.handle("face:findPotentialMatches", async (_event, { faceIds, threshold }) => {
    try {
      const { findPotentialMatches: findPotentialMatches2 } = await Promise.resolve().then(() => db$1);
      return findPotentialMatches2(faceIds, threshold);
    } catch (e) {
      return { success: false, error: String(e) };
    }
  });
  ipcMain$1.handle("debug:getBlurStats", async () => {
    const db2 = getDB();
    try {
      const stats = db2.prepare(`
        SELECT 
  ...
  ipcMain$1.handle("db:getPhotosMissingBlurScores", async () => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare("SELECT DISTINCT photo_id FROM faces WHERE blur_score IS NULL");
      const rows = stmt.all();
      return { success: true, photoIds: rows.map((r) => r.photo_id) };
    } catch (e) {
      return { success: false, error: String(e) };
    }
  });
  ...
  ipcMain$1.handle("db:autoAssignFaces", async (_event, { faceIds, threshold }) => {
    ...
    logger.info(`[Main] db:autoAssignFaces called with ${(faceIds == null ? void 0 : faceIds.length) || "ALL"} faces. Using Threshold: ${finalThreshold}`);
    try {
      return await autoAssignFaces2(faceIds, finalThreshold);
    } catch (e) {
  ...
  ipcMain$1.handle("settings:cleanupPreviews", async (_event, { days }) => {
    try {
      const previewDir = path.join(getLibraryPath(), "previews");
      try {
        await fs.access(previewDir);
  ...
  ipcMain$1.handle("face:deleteFaces", async (_event, faceIds) => {
    const db2 = getDB();
    const deleteParams = faceIds.map(() => "?").join(",");
    const stmt = db2.prepare(`DELETE FROM faces WHERE id IN (${deleteParams})`);
    stmt.run(...faceIds);
    return true;
  });
  ipcMain$1.handle("ai:getClusteredFaces", async () => {
    try {
      const { getFacesForClustering: getFacesForClustering2 } = await Promise.resolve().then(() => db$1);
      const crypto2 = await import("node:crypto");
      logger.info("[IPC] Starting Full Face Clustering...");
      const dbRes = getFacesForClustering2();
      ...
      const tempFile = path.join(app$1.getPath("temp"), `cluster_payload_${reqId}.json`);
  ...
  ipcMain$1.handle("ai:rotateImage", async (_, { photoId, rotation }) => {
    const db2 = getDB();
    ...
    try {
      ...
      if (photo && photo.file_path) {
        ...
        const result = await new Promise((resolve2, reject) => {
          scanPromises.set(photoId, { resolve: resolve2, reject });
          setTimeout(() => {
        ...
        if (result.success) {
          ...
          db2.prepare("UPDATE photos SET width = ?, height = ?, preview_cache_path = ? WHERE id = ?").run(result.width, result.height, previewPath, photoId);
          try {
            const faces = db2.prepare("SELECT id, box_json, person_id FROM faces WHERE photo_id = ?").all(photoId);
            ...
            if (faces.length > 0) {
              ...
              const transformedOldFaces = faces.map((face) => {
                try {
                  ...
                  if (Array.isArray(box) && box.length === 4) {
                    [x1, y1, x2, y2] = box;
                  } else if (box && typeof box.x === "number") {
                    x1 = box.x;
                    y1 = box.y;
                    x2 = box.x + box.width;
                    y2 = box.y + box.height;
              ...
              const scanResult = await new Promise((resolve2, reject) => {
                scanPromises.set(scanReqId, { resolve: resolve2, reject });
                setTimeout(() => {
                  if (scanPromises.has(scanReqId)) {
                    scanPromises.delete(scanReqId);
                    reject("Timeout");
                  }
                }, 3e5);
              });
              if (scanResult.success && scanResult.faces) {
                db2.prepare("DELETE FROM faces WHERE photo_id = ?").run(photoId);
                const insert = db2.prepare("INSERT INTO faces (photo_id, box_json, descriptor, score, blur_score, person_id) VALUES (?, ?, ?, ?, ?, ?)");
                let migratedCount = 0;
                ...
                for (const newFace of scanResult.faces) {
                  ...
                  if (transformedOldFaces.length > 0) {
                    ...
                    transformedOldFaces.forEach((old) => {
                      ...
                      const dist2 = Math.sqrt(Math.pow(cx - ocx, 2) + Math.pow(cy - ocy, 2));
                      const threshold = Math.min(result.width, result.height) * 0.08;
                      if (dist2 < bestDist && dist2 < threshold) {
                        bestDist = dist2;
                        matchedPid = old.person_id;
                        bestOldFaceIndex = old.id;
                      }
                    });
                  }
  ...
  ipcMain$1.handle("ai:generateTags", async (_, { photoId }) => {
    const db2 = getDB();
  ...
  ipcMain$1.handle("ai:analyzeImage", async (_, { photoId, scanMode, enableVLM }) => {
    const db2 = getDB();
  ...
  ipcMain$1.handle("ai:addFacesToVectorIndex", async (_, { vectors, ids }) => {
    return new Promise((resolve2, reject) => {
      const reqId = Math.floor(Math.random() * 1e6);
      scanPromises.set(reqId, { resolve: (r) => resolve2(r), reject });
      sendToPython({
        type: "add_to_index",
        payload: { vectors, ids, reqId }
      });
      setTimeout(() => {
        if (scanPromises.has(reqId)) {
  ...
  ipcMain$1.handle("ai:rebuildIndex", async () => {
    const db2 = getDB();
    ...
    try {
      ...
      return new Promise((resolve2, reject) => {
        const requestId = Math.floor(Math.random() * 1e6);
        scanPromises.set(requestId, {
          resolve: (res) => resolve2({ success: true, count: res.count }),
          reject
        });
        sendToPython({
          type: "rebuild_index",
  ...
  ipcMain$1.handle("ai:command", async (_, command) => {
    try {
      ...
      return new Promise((resolve2, reject) => {
        scanPromises.set(requestId, { resolve: resolve2, reject });
  ...
  ipcMain$1.handle("ai:clusterFaces", async (_, { faceIds, eps, min_samples } = {}) => {
    const db2 = getDB();
    try {
      let rows;
      if (faceIds && faceIds.length > 0) {
        const placeholders = faceIds.map(() => "?").join(",");
        const stmt = db2.prepare(`SELECT id, descriptor FROM faces WHERE id IN (${placeholders})`);
        rows = stmt.all(...faceIds);
      } else {
        const stmt = db2.prepare("SELECT id, descriptor FROM faces WHERE person_id IS NULL AND is_ignored = 0 AND descriptor IS NOT NULL");
        rows = stmt.all();
      }
      const descriptors = rows.map((r) => {
        if (!r.descriptor) return [];
  ...
  ipcMain$1.handle("db:addTags", async (_, { photoId, tags }) => {
    const db2 = getDB();
    ...
    try {
      ...
      logger.error("Failed to add tags:", error2);
      return { success: false, error: error2 };
    }
  });
  ipcMain$1.handle("db:getTags", async (_, photoId) => {
    const db2 = getDB();
    ...
    try {
      ...
      return tags.map((t2) => t2.name);
    } catch (error2) {
      logger.error("Failed to get tags:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:clearAITags", async (_event) => {
    const db2 = getDB();
    try {
      db2.exec(`
      ...
      logger.error("Failed to clear AI tags:", error2);
      return { success: false, error: error2 };
    }
  });
  ipcMain$1.handle("db:getPhoto", async (_, photoId) => {
    const db2 = getDB();
    try {
      const photo = db2.prepare("SELECT * FROM photos WHERE id = ?").get(photoId);
      return photo || null;
    } catch (error2) {
      logger.error("Failed to get photo:", error2);
      return null;
    }
  });
  ipcMain$1.handle("db:getPhotos", async (_, { limit: limit2 = 50, offset = 0, filter = {} } = {}) => {
    const db2 = getDB();
    try {
  ...
  ipcMain$1.handle("db:getPhotosForRescan", async (_, { filter = {} } = {}) => {
    ...
    try {
      ...
      console.error("Failed to get photos for rescan:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:getAllTags", async () => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare(`
      ...
      console.error("Failed to get all tags:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:removeTag", async (_, { photoId, tag }) => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare(`
  ...
  ipcMain$1.handle("db:renamePerson", async (_, { personId, newName }) => {
    const db2 = getDB();
    const cleanName = newName.trim();
    if (!cleanName) return { success: false, error: "Name cannot be empty" };
    try {
      const targetPerson = db2.prepare("SELECT id FROM people WHERE name = ? COLLATE NOCASE").get(cleanName);
      if (targetPerson) {
        ...
        db2.transaction(() => {
          db2.prepare("UPDATE faces SET person_id = ? WHERE person_id = ?").run(targetPerson.id, personId);
          db2.prepare("DELETE FROM people WHERE id = ?").run(personId);
        })();
        return { success: true, merged: true, targetId: targetPerson.id };
      } else {
        console.log(`[Main] Renaming person ${personId} to ${cleanName}`);
        db2.prepare("UPDATE people SET name = ? WHERE id = ?").run(cleanName, personId);
  ...
  ipcMain$1.handle("db:updateFaces", async (_, { photoId, faces, previewPath, width, height, globalBlurScore }) => {
    const db2 = getDB();
    try {
      const getOldFaces = db2.prepare("SELECT id, box_json, person_id FROM faces WHERE photo_id = ?");
      const updateFace = db2.prepare("UPDATE faces SET box_json = ?, descriptor = ?, blur_score = ?, is_reference = ? WHERE id = ?");
      const insertFace = db2.prepare("INSERT INTO faces (photo_id, box_json, descriptor, person_id, blur_score, is_reference) VALUES (?, ?, ?, ?, ?, ?)");
      const deleteFace = db2.prepare("DELETE FROM faces WHERE id = ?");
      const getAllKnownPeople = db2.prepare("SELECT id, descriptor_mean_json FROM people WHERE descriptor_mean_json IS NOT NULL");
      const getPersonRefCount = db2.prepare("SELECT count(*) as count FROM faces WHERE person_id = ? AND is_reference = 1");
      const getWorstReference = db2.prepare("SELECT id, blur_score FROM faces WHERE person_id = ? AND is_reference = 1 ORDER BY blur_score ASC LIMIT 1");
      const downgradeReference = db2.prepare("UPDATE faces SET is_reference = 0, descriptor = NULL WHERE id = ?");
      const updatePhotoPreview = db2.prepare("UPDATE photos SET preview_cache_path = ? WHERE id = ?");
      const updatePhotoDims = db2.prepare("UPDATE photos SET width = ?, height = ? WHERE id = ?");
      const updatePhotoBlur = db2.prepare("UPDATE photos SET blur_score = ? WHERE id = ?");
      const insertedIds = [];
      const transaction = db2.transaction(() => {
        if (previewPath) {
        ...
        if (globalBlurScore !== void 0 && globalBlurScore !== null) {
          updatePhotoBlur.run(globalBlurScore, photoId);
        }
        const oldFaces = getOldFaces.all(photoId);
        const usedOldFaceIds = /* @__PURE__ */ new Set();
        const peopleToUpdate = /* @__PURE__ */ new Set();
        for (const newFace of faces) {
          ...
          if (bestMatchId) {
            ...
            const personId = oldFace.person_id;
            let isRef = 0;
            let finalDesc = descBuf;
            if (personId) {
              const countObj = getPersonRefCount.get(personId);
              if (countObj.count < 100) {
                isRef = 1;
              } else {
                const worst = getWorstReference.get(personId);
            ...
            insertedIds.push(bestMatchId);
          } else {
            let matchedPersonId = null;
            let bestDistance = Infinity;
            const settings = getAISettings();
            const threshold = settings.faceSimilarityThreshold || 0.65;
            const knownPeople = getAllKnownPeople.all().map((p) => ({
              id: p.id,
              name: "",
              // Name not strictly needed for ID match, but kept for type safety
              mean: JSON.parse(p.descriptor_mean_json)
            }));
            ...
            let finalDesc = descBuf;
            if (matchedPersonId) {
              const countObj = getPersonRefCount.get(matchedPersonId);
              if (countObj.count < 100) {
                isRef = 1;
              } else {
                const worst = getWorstReference.get(matchedPersonId);
                if (worst && (newFace.blur_score || 0) > (worst.blur_score || 0)) {
                  downgradeReference.run(worst.id);
                  isRef = 1;
                } else {
      ...
      console.error("Failed to update faces:", error2);
      return { success: false, error: String(error2) };
    }
  });
  ipcMain$1.handle("db:getFaces", async (_, photoId) => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare(`
      ...
      console.error("Failed to get faces:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:getPeople", async () => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare(`
        WITH BestFaces AS (
      ...
      console.error("Failed to get people", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:getAllFaces", async (_, { limit: limit2 = 100, offset = 0, filter = {} } = {}) => {
    const db2 = getDB();
    try {
      let query = `
      ...
      console.error("Failed to get all faces:", error2);
      return [];
    }
  });
  ipcMain$1.handle("db:assignPerson", async (_, { faceId, personName }) => {
    ...
    const transaction = db2.transaction(() => {
      ...
      if (oldPersonId) {
        scheduleMeanRecalc(db2, oldPersonId);
      }
      return person;
    });
    try {
  ...
  ipcMain$1.handle("db:getLibraryStats", async () => {
    const db2 = getDB();
    const path2 = await import("node:path");
    try {
      try {
        db2.function("DIRNAME", (p) => path2.dirname(p));
        db2.function("EXTNAME", (p) => path2.extname(p).toLowerCase());
      } catch (e) {
      }
      const totalPhotosObj = db2.prepare("SELECT COUNT(*) as count FROM photos").get();
  ...
  ipcMain$1.handle("db:deleteFaces", async (_, faceIds) => {
    const db2 = getDB();
    try {
      ...
      const stmt = db2.prepare(`DELETE FROM faces WHERE id IN (${placeholders})`);
      const transaction = db2.transaction(() => {
        stmt.run(...faceIds);
      ...
      logger.error("Failed to delete faces:", error2);
      return { success: false, error: error2 };
    }
  });
  ...
  ipcMain$1.handle("db:clearScanErrors", async () => {
    const db2 = getDB();
    try {
      db2.exec("DELETE FROM scan_errors");
      return { success: true };
    } catch (e) {
      return { success: false, error: e };
    }
  });
  ipcMain$1.handle("db:retryScanErrors", async () => {
    const db2 = getDB();
    try {
      const stmt = db2.prepare("SELECT photo_id FROM scan_errors");
      const rows = stmt.all();
      db2.exec("DELETE FROM scan_errors");
      if (rows.length === 0) return [];
      const ids = rows.map((r) => r.photo_id);
      const placeholders = ids.map(() => "?").join(",");
      const photosStmt = db2.prepare(`SELECT * FROM photos WHERE id IN (${placeholders})`);
...
ipcMain$1.handle("settings:getLibraryPath", () => {
  return LIBRARY_PATH;
});
ipcMain$1.handle("settings:moveLibrary", async (_, newPath) => {
...
ipcMain$1.handle("db:unassignFaces", async (_, faceIds) => {
  const db2 = getDB();
  try {
    if (!faceIds || faceIds.length === 0) return { success: true };
    const placeholders = faceIds.map(() => "?").join(",");
...
ipcMain$1.handle("db:getFacesByIds", async (_, ids) => {
  try {
    const { getFacesByIds: getFacesByIds2 } = await Promise.resolve().then(() => db$1);
    return getFacesByIds2(ids);
  } catch (e) {
    logger.error("Failed to get faces by IDs:", e);
    return { success: false, error: String(e) };
  }
});
ipcMain$1.handle("db:getPerson", async (_, personId) => {
  const db2 = getDB();
  try {
...
ipcMain$1.handle("db:getFolders", async () => {
  ...
  try {
    ...
    console.error("Failed to get folders:", error2);
    return [];
  }
});
ipcMain$1.handle("db:ignoreFace", async (_, faceId) => {
  const db2 = getDB();
  try {
    const getFace = db2.prepare("SELECT person_id FROM faces WHERE id = ?");
    const face = getFace.get(faceId);
    const stmt = db2.prepare("UPDATE faces SET is_ignored = 1 WHERE id = ?");
    const transaction = db2.transaction(() => {
      stmt.run(faceId);
      if (face && face.person_id) {
        scheduleMeanRecalc(db2, face.person_id);
      }
    });
    transaction();
    return { success: true };
...
ipcMain$1.handle("db:ignoreFaces", async (_, faceIds) => {
  const db2 = getDB();
  try {
    if (!faceIds || faceIds.length === 0) return { success: true };
    const placeholders = faceIds.map(() => "?").join(",");
...
const pendingMeanRecalcs = /* @__PURE__ */ new Map();
const scheduleMeanRecalc = (db2, personId) => {
  ...
  const timeout = setTimeout(() => {
    ...
    try {
      ...
      recalculatePersonMean(db2, personId);
    } catch (e) {
...
ipcMain$1.handle("settings:getAIQueue", () => {
  return store.get("ai_queue", []);
});
ipcMain$1.handle("settings:setAIQueue", (_, queue) => {
...
ipcMain$1.handle("db:getAllUnassignedFaceDescriptors", async () => {
  const db2 = getDB();
  try {
    const stmt = db2.prepare(`
...
ipcMain$1.handle("db:factoryReset", async () => {
  const { getDB: getDB2 } = await Promise.resolve().then(() => db$1);
  const fs2 = await import("node:fs/promises");
  const path2 = await import("node:path");
  const db2 = getDB2();
  try {
    ...
    try {
      await fs2.rm(previewDir, { recursive: true, force: true });
      await fs2.mkdir(previewDir, { recursive: true });
...
ipcMain$1.handle("db:getUnprocessedItems", async () => {
  ...
  try {
    ...
    console.error("Failed to get unprocessed items:", error2);
    return [];
  }
});
ipcMain$1.handle("os:getLogPath", () => {
  return logger.getLogPath();
});
ipcMain$1.handle("os:showInFolder", (_, path2) => {
  shell$1.showItemInFolder(path2);
});
ipcMain$1.handle("os:openFolder", (_, path2) => {
  shell$1.openPath(path2);
});
