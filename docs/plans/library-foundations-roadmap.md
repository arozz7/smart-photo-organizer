...
2. **Versioned migrations.** From Phase 128 on, every *new* schema change is a numbered migration tracked by `PRAGMA user_version`. The existing try/catch `ALTER TABLE` block is frozen as the **legacy baseline**: it keeps running idempotently on every start exactly as before, outside the runner (see Phase 128). Existing libraries have `user_version = 0` until their first numbered migration.
...
1. **Baseline:** Run the full suite and record the pass/fail baseline in the changelog. As of the pre-roadmap release, TypeScript is 419/419 green and Python is 172 passing with **6 failures that already existed at v0.8.0**:
   ...
   **DONE:** all fixed (one real bug in the HTTP API route, three stale tests, one stale-mock integration test). The Python suite is now 183/183 green, so every later phase starts from an all-green baseline. See `aiChangeLog/phase-128-versioned-migrations.md`.
2. **Legacy fixture (created *before* any code moves) — DONE:** `tests/fixtures/legacy-v0.8.1.db` (plus a schema/row-count snapshot), generated by `scripts/fixtures/build-legacy-db.ts` (run through `tests/tools/buildLegacyFixture.test.ts` with `BUILD_LEGACY_FIXTURE=1`) using the real, unmodified `initDB`, seeded with named people, eras, faces (confirmed, ignored, bucketed), tags, smart albums, duplicate groups and scan errors. Named for v0.8.1 because that is the release it was built from; the schema is identical to v0.8.0.
   - **Real-world drift check — DONE:** Schemas of the DEV and PROD libraries were read read-only (schema only, no row data, no copy) and compared with the fixture: **no differences** in tables, columns or indexes; both have `user_version = 0`.
3. **Refactor (location change only) — DONE:** The legacy block moved verbatim from `electron/db.ts` into `electron/data/migrations/legacyBaseline.ts` (`applyLegacyBaseline`), and `parseExifDate` into `electron/utils/exifDate.ts` (re-exported from `db.ts`). `db.ts` went from 934 to ~66 lines. A line-by-line diff confirms the body is identical apart from three type-only casts. **Design change:** the baseline is *not* a runner migration, because it is async, runs `VACUUM` and toggles `PRAGMA foreign_keys`, none of which are valid inside a migration transaction. The runner handles only new, additive, synchronous migrations.
4. **Migration runner — DONE:** `electron/data/migrations/MigrationRunner.ts` reads `user_version`, runs pending numbered migrations from the registry in `electron/data/migrations/index.ts` (`MIGRATIONS`, currently empty), each in its own transaction, and bumps `user_version`.
5. **Backup — DONE:** `electron/data/migrations/DatabaseBackup.ts` (SQLite online backup into `<library>/backups/`, retention of 3, never prunes files it did not create). One backup is taken before the first pending migration, and the run aborts (fails closed) if it cannot be written.
6. **Downgrade guard — DONE (runner):** If `user_version` is greater than the app's highest known migration, the runner logs a warning, applies nothing and continues. *Remaining:* show a non-blocking notice in the UI.
...
**Backward compat:** No schema change in this phase, and no user-visible change. Existing libraries keep `user_version = 0`, no backup is taken and no `backups/` folder is created until the first numbered migration (Phase 130) is pending.
