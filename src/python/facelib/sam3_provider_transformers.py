"""
SAM 3 implementation of SegmentationProvider using the transformers integration.

CPU-only fallback: Meta's official `sam3` package (see sam3_provider.py,
Sam3PackageProvider) has a confirmed device-handling bug in its text/exemplar
prompt path that breaks on CPU-only machines (a hardcoded CUDA reference
independent of the device parameter). This transformers-based implementation
works correctly on CPU, so segmentation_factory.py routes CPU-only machines
here and GPU machines to Sam3PackageProvider.
...
Configuration (via ai-config.json):
  segmentation.model_checkpoint_cpu — path to weights file or HF repo dir
...
class Sam3TransformersProvider(SegmentationProvider):
    """
