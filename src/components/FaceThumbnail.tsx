...
const FaceThumbnail = React.memo<FaceThumbnailProps & { fallbackSrc?: string }>(function FaceThumbnail({ src, fallbackSrc, box, originalImageWidth, alt, className, useServerCrop }: FaceThumbnailProps & { fallbackSrc?: string }) {
    ...
    return (
        <div className={`overflow-hidden relative bg-gray-900 ${className || ''}`} style={{ contentVisibility: 'auto', containIntrinsicSize: '300px' }}>
            {isLoading && (
            ...
            <img
                ...
                src={(() => {
                    ...
                    if (useServerCrop) {
                        ...
                        // Request standard thumbnail size to prevent loading full res crops for large faces
                        finalSrc += `&hq=true&width=300`;
                    }
