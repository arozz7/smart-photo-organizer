...
try {
    ...
    results.forEach(r => {
        ...
        // Calculate Hash of LOWERCASE path (just in case)
        const hashLower = crypto.createHash('md5').update(r.file_path.toLowerCase()).digest('hex');
        console.log(`Possible Hash (md5(lowercase)): ${hashLower}`);
    });
